...
# Step 1: Prepare for the individual data to generate the summary statistics of SNP-Outcome association in the sub-sample of UKB without AD sleep data
## Inport the individual genotype data of AD sleepduration from the UK Biobank (We extracted the individual genotype data of AD sleep duration previously)
ad_sleepduration_genotype<-read.csv("ad_sleepduration_genotype.csv")

## Inport the UKB data with HbA1c/glucose and other related covariates (age, sex, assessment centre, genotyping chip, PC1 - PC40) (would additionally include fasting time and dilution factor for non-fasting glucose)
...
## Merge genotype data with UKB data  
data2_ukb_hba1c<-merge(data_ukb_hba1c, ad_sleepduration_genotype, by = "projectid")
data2_ukb_hba1c<-na.omit(data2_ukb_hba1c)


## exclude participants in the AD sleep GWAS
data_ad_gwas<-read.csv("data_ad_gwas.csv")
sample2_ad_sleepduration_hba1c <- data2_ukb_hba1c[ !(data2_ukb_hba1c$projectid %in% c(data_ad_gwas$projectid)), ]





# Step 2: Generate the summary statistics for SNP-Outcome association in the sub-sample of UKB without AD sleep data
n <- nrow(sample2_ad_sleepduration_hba1c)
G1.2 <- as.matrix(sample2_ad_sleepduration_hba1c[,-(1:46)])
Y <- as.matrix(sample2_ad_sleepduration_hba1c[,2])

## covariates
sex<-as.matrix(sample2_ad_sleepduration_hba1c[,3])
age<-as.matrix(sample2_ad_sleepduration_hba1c[,4])
centre<-as.matrix(sample2_ad_sleepduration_hba1c[,5])
chip<-as.matrix(sample2_ad_sleepduration_hba1c[,6])

pc1 <- as.matrix(sample2_ad_sleepduration_hba1c[,7])     
pc2 <- as.matrix(sample2_ad_sleepduration_hba1c[,8]) 
pc3 <- as.matrix(sample2_ad_sleepduration_hba1c[,9]) 
pc4 <- as.matrix(sample2_ad_sleepduration_hba1c[,10]) 
pc5 <- as.matrix(sample2_ad_sleepduration_hba1c[,11]) 
pc6 <- as.matrix(sample2_ad_sleepduration_hba1c[,12]) 
pc7 <- as.matrix(sample2_ad_sleepduration_hba1c[,13]) 
pc8 <- as.matrix(sample2_ad_sleepduration_hba1c[,14]) 
pc9 <- as.matrix(sample2_ad_sleepduration_hba1c[,15]) 
pc10 <- as.matrix(sample2_ad_sleepduration_hba1c[,16]) 

pc11 <- as.matrix(sample2_ad_sleepduration_hba1c[,17])     
pc12 <- as.matrix(sample2_ad_sleepduration_hba1c[,18]) 
pc13 <- as.matrix(sample2_ad_sleepduration_hba1c[,19]) 
pc14 <- as.matrix(sample2_ad_sleepduration_hba1c[,20]) 
pc15 <- as.matrix(sample2_ad_sleepduration_hba1c[,21]) 
pc16 <- as.matrix(sample2_ad_sleepduration_hba1c[,22]) 
pc17 <- as.matrix(sample2_ad_sleepduration_hba1c[,23]) 
pc18 <- as.matrix(sample2_ad_sleepduration_hba1c[,24]) 
pc19 <- as.matrix(sample2_ad_sleepduration_hba1c[,25]) 
pc20 <- as.matrix(sample2_ad_sleepduration_hba1c[,26]) 

pc21 <- as.matrix(sample2_ad_sleepduration_hba1c[,27])     
pc22 <- as.matrix(sample2_ad_sleepduration_hba1c[,28]) 
pc23 <- as.matrix(sample2_ad_sleepduration_hba1c[,29]) 
pc24 <- as.matrix(sample2_ad_sleepduration_hba1c[,30]) 
pc25 <- as.matrix(sample2_ad_sleepduration_hba1c[,31]) 
pc26 <- as.matrix(sample2_ad_sleepduration_hba1c[,32]) 
pc27 <- as.matrix(sample2_ad_sleepduration_hba1c[,33]) 
pc28 <- as.matrix(sample2_ad_sleepduration_hba1c[,34]) 
pc29 <- as.matrix(sample2_ad_sleepduration_hba1c[,35]) 
pc30 <- as.matrix(sample2_ad_sleepduration_hba1c[,36]) 

pc31 <- as.matrix(sample2_ad_sleepduration_hba1c[,37])     
pc32 <- as.matrix(sample2_ad_sleepduration_hba1c[,38]) 
pc33 <- as.matrix(sample2_ad_sleepduration_hba1c[,39]) 
pc34 <- as.matrix(sample2_ad_sleepduration_hba1c[,40]) 
pc35 <- as.matrix(sample2_ad_sleepduration_hba1c[,41]) 
pc36 <- as.matrix(sample2_ad_sleepduration_hba1c[,42]) 
pc37 <- as.matrix(sample2_ad_sleepduration_hba1c[,43]) 
pc38 <- as.matrix(sample2_ad_sleepduration_hba1c[,44]) 
pc39 <- as.matrix(sample2_ad_sleepduration_hba1c[,45]) 
pc40 <- as.matrix(sample2_ad_sleepduration_hba1c[,46]) 
...
## Outcome: HbA1c
### data_sample2 as obtained in Step 2
### Assign the other allele to the data_sample2 because the individual genotype data has no information of other allele
oa<-select(ad_sleepduration, SNP, other_allele.exposure)
...
### Harmonise data, action = 1. Assume all alleles are presented on the forward strand because the SNP-exposure and SNP-outcome associations are both from the UKB). As such, there is no need to remove palindromic SNPs
harmonised_ad_sleepduration_hba1c<-harmonise_data(exposure_dat = ad_sleepduration, outcome_dat = data_sample2_format, action = 1) 
